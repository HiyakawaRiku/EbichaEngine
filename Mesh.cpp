...
void Mesh::Draw(UINT vertexCountPerInstance)
{
	...
	dxCommon_->GetCommandList()->SetGraphicsRootDescriptorTable(2, dxCommon_->textureSrvHandleGPU[0]);
...
void Mesh::DrawSphere(UINT vertexCountPerInstance,bool useMonsterBall)
{
	...
	dxCommon_->GetCommandList()->SetGraphicsRootConstantBufferView(1, wvpResource->GetGPUVirtualAddress());
	// SRVのDescriptorTableの先頭を設定。2はrootParameter[2]である。
	dxCommon_->GetCommandList()->SetGraphicsRootDescriptorTable(2, useMonsterBall?dxCommon_->textureSrvHandleGPU[0] : dxCommon_->textureSrvHandleGPU[1]);
	// 描画！（DrawCall/ドローコール）。3頂点で1つのインスタンス。インスタンスについては今後
